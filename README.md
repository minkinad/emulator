...
Реализованы ядро CPU, ассемблер и консольные примеры. Браузерный интерфейс находится в разработке. Документация доступна на [GitHub Pages](https://minkinad.github.io/emulator/).

## Быстрый старт

Требуются Node.js 22+ и npm. Из корня репозитория:
...
Пример собирает программу с меткой и циклом, печатает машинный листинг и вычисляет `3 + 2 + 1 = 6`. Результат сохраняется в памяти данных по адресу `0x0300`.

Для демонстрации ручных машинных слов: `npm run core:demo`.
...
| `npm run check` | Проверка типов, тесты, сборка модулей и документации. |
...
tests/                 Проверки ядра и ассемблера
...
Ядро и ассемблер написаны на TypeScript и не зависят от React, DOM или Node.js. Консольные примеры используют Node.js; документация — VitePress. Для будущего браузерного интерфейса выбран React + Vite.
