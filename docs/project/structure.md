...
│   ├── programs/               # Каталог массивов, готовые слова результата
│   │   ├── catalog.ts          # Описание примеров и начальные данные
│   │   ├── result.ts           # Знаковое отображение двух/трёх слов
│   │   └── sources.generated.ts # Модуль из ASM; проверяется тестами
...
│   ├── programs/               # Интеграция массивов и независимый эталон
│   └── browser/                # Сценарии Playwright в Chromium
├── examples/
│   ├── assembly/               # Исходники .asm и запуск через CPU
│   └── machine/                # Ручной пример add.mjs и листинги sum.txt/dot.txt
├── docs/
...
| Программы массивов | `examples/assembly/sum.asm`, `dot.asm`; каталог `src/programs/`, тесты `tests/programs/`. |
...
`src/programs/sources.generated.ts` и машинные листинги — проверяемые артефакты в Git. Исходные `.asm` остаются единственным редактируемым источником программ. Порядок обновления описан в [руководстве](../architecture/programs.md).
